...
[为什么要精确](#为什么必须精确计数) · [工作原理](#工作原理) · [安装](#安装) · [使用](#使用) · [CI 预算闸](#在-ci-中强制-context-预算) · [路线图](#路线图)
...
- **Lint** —— 给 MCP **server 作者**的可执行建议：过长的 description、冗余 schema、“这里可省约 400 token”。
- **Tool Search 建模** —— `deferred` 与 `alwaysLoad` 对比，看清哪些是预先付费、哪些是按需加载。
